function Intensity = fitFunctionYeColor_2_QuarterWaveplate(x, polarizerPosition, analyzerPosition, wavelength,I0,equivalentThickness)
...
analyzerPosition = rad2deg(analyzerPosition);%+x(2);


delmeFac = 0;

for aLoop = 1:length(analyzerPosition)
%     Delta = (tf(aLoop)*0.05*1e-6*2*pi./wavelength(aLoop));
    Delta = pi/2;
    ...
    delmeRight = [cosd(delmeFac) sind(delmeFac) ; -sind(delmeFac) cosd(delmeFac)];
    ...
    delmeLeft = [cosd(-delmeFac) sind(-delmeFac) ; -sind(-delmeFac) cosd(-delmeFac)];
    
    Ts = delmeLeft*RmatRight*Lmat*RmatLeft*RmatLeft*Lmat*RmatRight*delmeRight;
