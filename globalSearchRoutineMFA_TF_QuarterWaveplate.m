function [x, fitFcn, costFcn] = globalSearchRoutineMFA_TF_QuarterWaveplate(IntensitySweep, I0, x0, ctrl)
...
elseif strcmp(ctrl.opticalModel,'QuarterWaveplate')
%     equivalentThickness = ;
    fitFcn  = @(x, polarizerPosition, analyzerPosition, wavelength, I0,equivalentThickness) fitFunctionYeColorQuarterWaveplate(x, polarizerPosition, analyzerPosition, wavelength,I0,equivalentThickness);
else
    disp('Model to fit undefined. Set ctrl.opticalModel according to instructions.')
    error('Model underfined')
end

costFcn = @(x) sqrt(1/size(IntensitySweep,1)*sum((fitFcn(x,IntensitySweep(:,3),IntensitySweep(:,4),IntensitySweep(:,5),I0,IntensitySweep(:,5)./(4*0.05*1e-6*2*pi)) -  IntensitySweep(:,6)).^2));
...
if license('test', 'Optimization_Toolbox')
    gs = GlobalSearch;
    problem = createOptimProblem('fmincon','x0',x0,'objective',costFcn,'lb',[-40],'ub',[40]);
    x = run(gs,problem);
else
